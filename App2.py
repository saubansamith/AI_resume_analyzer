...
class SQLHandle:
    def insert_data(self,name,email,res_score,timestamp,no_of_pages,reco_field,cand_level,skills,recommended_skills,courses):
        ...
        rec_values = (name, email, str(res_score), timestamp,str(no_of_pages), reco_field, cand_level, skills,recommended_skills.strip('[]').replace("'",''),courses.strip('[]').replace("'",''))
        cursor.execute(insert_sql, rec_values)
        connection.commit()

    st.set_page_config(
    page_title="AI Resume Analyzer",
    page_icon='./logo/resume_logo.png',
    )
class ResumeAnalyzer:
    def run(self):
        img = Image.open('./Logo/resume_logo.png')
        # cropped_img = img.crop((0, 100, 500, 600))  # (left, top, right, bottom)

        # Resize to fit nicely in Streamlit app
        resized_img = img.resize((500, 180))  # adjust as needed
        ...
        if choice == 'User':
            ...
            if pdf_file is not None:
                ...
                web_keyword = ['react', 'django', 'node jS', 'react js', 'php', 'laravel', 'magento', 'wordpress',
                                'javascript','JavaScript', 'angular js', 'c#', 'flask','html', 'HTML', 'CSS', 'css', 'js', 'Bootstrap', 'bootstrap', 'jquery', 'ajax',
                                'json', 'xml', 'rest api', 'api', 'web development', 'web developer', 'web designer',
                                'web application', 'web app', 'web design', 'web programming', 'web technologies', 'web services',
                                'web development frameworks', 'web development tools', 'web development languages']
                android_keyword = ['android','android development','flutter','kotlin','xml','kivy','git','sdk','sqlite','android studio','java','android sdk','android app development','android programming','android developer',
                                'android application development','android software development','android mobile development','android web development','android game development','android app design','android app programming']
                ios_keyword = ['ios','ios development','swift','cocoa','cocoa touch','xcode','objective-c','sqlite','plist','storekit','ui-kit','av foundation','auto-layout','ios app development','ios programming','ios developer']
                ...
                for i in skills:
                    ...
                    ## Ui-UX Recommendation
                        ...
                        break
                if pdf_file:
                    if st.session_state.ran==0:
                        st.session_state.ran=1
                        jobs=get_links(reco_field)
                        st.markdown('''
            ...
            if st.button('Login'):
                if ad_user == 'admin' and ad_password == 'admin':
                    ...
                    df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page',
                                                    ...
                                                    'Recommended Course']).sort_values(by='ID',ascending=False)
                    ...
                    df = pd.DataFrame(data, columns=['ID','Name','Email','Rating','Review','Timestamp']).sort_values(by='ID',ascending=False)
                    st.dataframe(df,hide_index=True)
                    st.markdown(BackgroundProcess().get_table_download_link(df,'User_Data.csv','Download Report'), unsafe_allow_html=True)
